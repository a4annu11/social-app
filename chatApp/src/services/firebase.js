...
// Auth (adapted for custom token auth; backend handles register/login)
...
export const incrementUnreadCount = async (
  ...
  try {
    ...
    participants.forEach(uid => {
      if (uid !== senderUid) {
        updates[`unreadCount.${uid}`] = firestore.FieldValue.increment(1);
      }
    });
...
export const initializeChatDoc = async (chatId, participants) => {
  try {
